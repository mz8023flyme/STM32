	...
	{   
		...
		if(Turn_Off(Voltage)==0)               //===如果电池电压不存在异常
		{
			if(Flag_Way==1)     //进行运动学分析之后的控制
			{
			 ...
			 temp=Kinematic_Analysis(TargetX,TargetY,Target_Beta,Target_Alpha);  
			}
			else	
			{
...
{	
    ...
    if(Target1<Amplitude_L)  Target1=Amplitude_L;	
	  ...
	  if(Target5<Amplitude_L)  Target5=Amplitude_L;	
		...
		if(Target6<300)  				 Target6=300;	
		if(Target6>1200)  			 Target6=1200;	
}
...
{ 	
	 ...
	 else if(PS2_KEY==6)		Target6+=Step*3000;     //预留
	 else if(PS2_KEY==8)		Target6-=Step*3000;
	 ...
	 else if(PS2_KEY==6)		Target6+=Step;  //预留
	 else if(PS2_KEY==8)		Target6-=Step;
		}
	     if(PS2_KEY==11)		Speed+=0.05;  //预留
