...
typedef struct
{
	float           AccX;				//X?á?ó?ù?è
	float		AccY;				//Y?á?ó?ù?è
	float		AccZ;
}MPU6050_AccTypeDef;
...
{
	float		Angle_FY;		//???????è
	float		Angle_HG;		//oá1????è
	float		Angle_HX;		//o??ò???è
}MPU6050_GestureTypeDef;


//模块的A0引脚接GND，IIC的7位地址为0x68，若接到VCC，需要改为0x69
#define MPU6050_SLAVE_ADDRESS  (0x68<<1)      //MPU6050器件读地址
/* 设置mpu加速度的范围模式，其值可以是 0 1 2 3 */
#define MPU6050_Set_RA_Acc              0
/* 设置mpu加速度的范围模式，其值可以是 0 1 2 3 */
#define MPU6050_Set_RA_Gyro             0             
